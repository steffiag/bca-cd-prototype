...
app.put("/misdemeanors/:id", async (req, res) => {
  try {
    const { id } = req.params;
    const { club_name, misdemeanor_type, notes } = req.body;
    const record = await Misdemeanor.findByPk(id);
    if (!record) return res.status(404).json({ error: "Not found" });
    await record.update({ club_name, misdemeanor_type, notes });
    res.json(record);
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: "Failed to update misdemeanor" });
  }
});
...
app.get("/teacher-availability", async (req, res) => {
  try {
    await syncTeachers();
    const dbTeachers = await db.Teacher.findAll();
    const wednesdayClubs = await db.WednesdayClub.findAll({ where: { status: "Approved" } });
    const teachers = dbTeachers.map((teacher) => ({
      id: teacher.id,
      name: teacher.name,
      email: teacher.email,
...
app.put("/teacher-availability/:id/confirm", async (req, res) => {
  try {
    ...
    const teacher = await db.Teacher.findByPk(id);
    if (!teacher) return res.status(404).json({ error: "Teacher not found" });
    await teacher.update({ available: available === true || available === "Yes" });
    res.json({ success: true, teacher });
  } catch (err) {
    console.error("Error confirming availability:", err);
    res.status(500).json({ error: err.message });
...
app.delete("/teacher-availability/:id/:source", async (req, res) => {
  try {
    const { id, source } = req.params;
    if (source === "google_form") {
      return res.status(400).json({ error: "Cannot delete teachers from Google Forms." });
    }
    const deleted = await db.Teacher.destroy({ where: { id: parseInt(id) } });
    if (deleted) {
      res.json({ success: true, message: "Teacher deleted" });
...
async function syncWednesdayClubs() {
  const responses = await getFormResponses("1a7PoNfDMwsEwFasPrA_6k8Fti4__E11xC32Eanchcc8");
  for (const resp of responses) {
    const responseId = resp.responseId;
    ...
    if (newClub.photo_file_id) downloadImage(newClub.photo_file_id, newClub.club_name);
  }
}
...
app.put("/wednesday-club/:id", async (req, res) => {
  try {
    ...
    const { club, email, category, advisor, room, members, status, mission } = req.body;
    const clubToUpdate = await db.WednesdayClub.findByPk(id);
    if (!clubToUpdate) return res.status(404).json({ error: "Club not found" });
    await clubToUpdate.update({
      club_name: club,
      leader_email: email,
      category,
...
app.delete("/wednesday-club/:identifier/:source", async (req, res) => {
  try {
    const { identifier, source } = req.params;
    if (source === "google_form") {
      return res.status(400).json({ error: "Cannot delete clubs from Google Forms." });
    }
    const whereClause = isNaN(identifier) ? { leader_email: identifier } : { id: parseInt(identifier) };
    const deleted = await db.WednesdayClub.destroy({ where: whereClause });
    if (deleted) {
      res.json({ success: true, message: "Club deleted" });
    } else {
...
app.get("/approved-wednesday-clubs", async (req, res) => {
  try {
    ...
    const clubs = await db.WednesdayClub.findAll({
      where: { status: "Approved" },
      attributes: ["id", "club_name", "mission", "photo_file_id"],
    });
    res.json(clubs.map(c => ({ dbId: c.id, club: c.club_name, mission: c.mission, photo: c.photo_file_id })));
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: "Failed to fetch approved Wednesday clubs" });
  }
...
// ============================================
// AI MERGE / SIMILARITY ENDPOINTS
// ============================================

app.get("/ai-merges", async (req, res) => {
  try {
    const formResponses = await getFormResponses("1fvK9FLMsuwixNsDF6vbG37H_IFpm-Kh7aTnYwKdgYCM");
    const formClubs = formResponses.map(resp => {
      const answers = resp.answers;
      return {
        club: answers["58d95ef3"]?.textAnswers.answers[0].value?.trim() || "",
...
app.get("/user-clubs/:user_id", async (req, res) => {
  try {
    ...
    const user = await db.User.findOne({ where: { usr_email: user_id } });
    if (!user) return res.json({ success: true, enrollments: [] });
    const enrollments = await db.ClubEnrollment.findAll({ where: { user_id: user.usr_id } });
    res.json({ success: true, enrollments });
  } catch (err) {
    console.error(err);
    res.status(500).json({ success: false, error: err.message });
...
app.get("/club/:club_id/members", async (req, res) => {
  try {
    ...
    const members = await db.ClubEnrollment.findAll({
      where: { club_id, club_type },
      include: [{ model: db.User, attributes: ["usr_first_name", "usr_last_name", "usr_email"] }],
    });
    res.json({ success: true, members });
  } catch (err) {
    console.error(err);
